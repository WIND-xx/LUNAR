...
static bool buzzer_beep_local(uint32_t duration_ms)
{
    if (!s_beep_timer) {
        ...
        if (xTimerIsTimerActive(s_beep_timer) != pdFALSE) {
...
static bool heat_create_freertos_resources(void)
{
    ...
    if (!s_heat_data.mutex) { LOG_PRINTF("Create heat mutex failed!\r\n"); return false; }
    ...
    if (!s_heat_data.ctrl_queue) {
        ...
        vSemaphoreDelete(s_heat_data.mutex); s_heat_data.mutex = NULL;
        return false;
    }

    s_heat_data.heating_timer = xTimerCreate("HeatTimer", pdMS_TO_TICKS(1000), pdFALSE, NULL, heating_timer_callback);
    if (!s_heat_data.heating_timer) {
        vQueueDelete(s_heat_data.ctrl_queue); s_heat_data.ctrl_queue = NULL;
        vSemaphoreDelete(s_heat_data.mutex); s_heat_data.mutex = NULL;
        return false;
    }

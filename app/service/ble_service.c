/**
 * @file ble_service.c
 * @brief BLE服务：帧分发 + AT命令 (CMSIS-RTOS v2)
 * @version 4.1
 */
...
static void ble_task(void *arg) {
    ...
    for (;;) {
        ...
        if ((s_stats.total % 60000) == 59999) {
            LOG_PRINTF("BLE: F=%lu M=%lu A=%lu U=%lu Free=%u\r\n", s_stats.total, s_stats.modbus, s_stats.at,
                       s_stats.unknown, osThreadGetStackSpace(osThreadGetId()));
        }
...
void ble_mode(bt_mode_t mode) {
    ...
    case BT_MODE_OFF:   c = "AT+CM08\r\n"; bsp_led_set_mode(g_led, BSP_LED_BT, BSP_LED_MODE_OFF, 0);
                        bsp_led_set_mode(g_led, BSP_LED_MUSIC, BSP_LED_MODE_OFF, 0); break;
    case BT_MODE_BT:    c = "AT+CM01\r\n"; bsp_led_set_mode(g_led, BSP_LED_BT, BSP_LED_MODE_ON, 0); break;
    case BT_MODE_MUSIC: c = "AT+CM04\r\n"; bsp_led_set_mode(g_led, BSP_LED_MUSIC, BSP_LED_MODE_ON, 0); break;
    default: return;
    }
...
void ble_service_init(void) {
    const osThreadAttr_t attr = {
        .name       = TASK_BLE_NAME,
        .stack_size = TASK_BLE_STACK,
        .priority   = (osPriority_t)TASK_BLE_PRIORITY,
    };
    osThreadNew(ble_task, NULL, &attr);
}

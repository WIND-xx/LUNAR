...
void bt401_init(void)
{
    ...
    HAL_StatusTypeDef status =
        HAL_UARTEx_ReceiveToIdle_DMA(&huart3, dma_buffer[current_dma_buf], BT401_DMA_BUFFER_SIZE);
    if (status != HAL_OK) {
        /* 初始化失败不阻塞系统，记录错误 */
        return;
    }
...
{
    if (xBt401TxMutex == NULL || buf == NULL || len == 0)
        return 0;
...
{
    if (xBt401ReadyQ == NULL || frame == NULL)
        return 0;

    uint8_t idx;
    if (xQueueReceive(xBt401ReadyQ, &idx, timeout) != pdTRUE)
        return 0;

    frame->len = buffer_lengths[idx];
    if (frame->len > BT401_DMA_BUFFER_SIZE)
        frame->len = BT401_DMA_BUFFER_SIZE;

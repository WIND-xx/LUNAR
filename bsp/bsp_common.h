 ...
 */
#define BSP_HANDLE_DECLARE(name) typedef struct bsp_##name##_s bsp_##name##_t
...
/** 限制值在 [min, max] 范围内 */
#define BSP_CLAMP(val, min, max) (((val) < (min)) ? (min) : (((val) > (max)) ? (max) : (val)))
